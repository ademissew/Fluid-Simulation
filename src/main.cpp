    ...
    for (int i = 0; i<size; ++i){
        for (int j = 0; j<size; ++j){
            for (int k = 0; k < size; ++k){
                ...
                if(d>0){
                    const Vector3f PARTICLE_COLOR = Vector3f(0.4f, 0.7f, 1.0f)*density[value(i,j,k)]*.4;
...
int main(int argc, char** argv)
{
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        for (int i = 0; i < size; ++i){
            for(int j = 0; j < size; ++j){
                addFluid(i,j,k,(i+j+5)*(size-i+1)/(15*t));
                adjustVelocity(i,j,k,(rand()%5)+5,(rand()%5)+5,0);
            }
            // mult += 0.;
        }
    ...
    //     for (int i = 0; i < size; ++i){
    //         for(int j = 0; j < size; ++j){
    //             if ((i == j || i - 1 == j || j - 1 == i) && (i == (int)size/2 + 1 || i === (int)size/2 -1)){
    //                 addFluid(i,j,k,(rand()%6)+1);
    //             adjustVelocity(i,j,k,(rand()%5)+5,(rand()%5)+5,0);
    //             } else {
    //                 addFluid(i,j,k,(rand()%1)+1);
    //                 adjustVelocity(i,j,k,(rand()%5)+5,(rand()%5)+5,0);
    //             }
    //     }
    // }
